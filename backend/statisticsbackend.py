...
def csv_listener_Post():
    ...
    if request_type == 'upload-data': #Upload Data
        ...
        try:
            frontend_upload_file(frontend_file)
            is_file_uploaded = True
            print("File Uploaded Successfully")
            print(frontend_file.filename)
            return jsonify({"Message": "CSV Data Received Successfully", "Data": frontend_file.filename}), 200
        except Exception as e:
            print(f"Error: {str(e)}")
            return jsonify({"Error": str(e)}), 500
    elif request_type == 'display-data': #Column1 Data Selection Update 
        ...
        return jsonify({ #Send JSON Filtered Data to Frontend
