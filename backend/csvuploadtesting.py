...
mp.use('Agg') # Use the 'Agg' backend for non-interactive mode
...
def frontend_upload_file(file, file_type):
    ...
    try:
        if file_type == ".csv":
            ...
            #CSV data indices selection
            data_indices = uploaded_data.index.tolist()
...
def frontend_analyze_data(column1, column2, column3, test, mean):
    ...
    #Mean Calculation
    if column1 and column1 in uploaded_data.columns and test == "Mean":

        #Try to calculate mean
        try:
            filtered_data1 = uploaded_data[column1].dropna() #Filter Data
            mean = np.mean(filtered_data1)
            print("Data Analyzed")
        ...
        try:
            ...
            print(f"Error analyzing data: {e}")
    elif column1 and column1 in uploaded_data.columns and column2 and column2 in uploaded_data.columns and test == "Independent T-Test": #Inpendent T-Test
        ...
        try:
            ...
            test_statistic, p_value = f_oneway(filtered_data1, filtered_data2, filtered_data3) #Run test and unpack results
            print("Data Analyzed")
...
#Frontend Display Data Function
def frontend_display_data(column1, column2, display):
    ...
    if column1 and column1 in uploaded_data.columns and display == "Q-Q Plot": #Q-Q Plot
        ...
        try:
            ...
            #Conver tthe buffer to a base64 string
    ...
    elif column1 and column1 in uploaded_data.columns and display == "Histogram": #Histogram
        #Try to Display Histogram
        try:
            df = pd.DataFrame(uploaded_data) #Dataframe
            ...
            plt.title("Histogram for " + column1)
            plt.xlabel(column1)
            plt.ylabel(column2)
    ...
    elif column1 and column1 in uploaded_data.columns and column2 and column2 in uploaded_data.columns and  display == "Box Plot": #Box Plot
        df = pd.DataFrame(uploaded_data) #Dataframe
        #Try to render box plot
        try:
            ...
            plt.title("Box Plot for " + column1)
            ...
            #Convert nthe buffer to a base64 string
            image_base64 = base64.b64encode(buf.getvalue()).decode('utf-8')
            return image_base64
        except Exception as e:
            print(f"Error displaying data: {e}")
    ...
    elif column1 and column1 in uploaded_data.columns and column2 and column2 in uploaded_data.columns and display == "Scatter Plot (With Regression)": #Scatter Plot with Regression
        df = pd.DataFrame(uploaded_data) #Dataframe
        #Try to Display Scatterplot
        try:
            print("Scatter Plot (With Regression) Rendered")
            display_figure = sns.lmplot(data = df, x = column1, y = column2, aspect = 2, height = 6) #Update Widget Storage Variable
...
def csv_listener_Post():
    ...
    if request_type == 'upload-data': #Upload Data
        ...
        #Try to upload data
        try:
            #Sort Through File Type and Pass File and File Type Accordingly
            if frontend_file_type == ".csv":
                ...
                return jsonify({"Message": ".csv File Data Received Successfully", "File Name": frontend_file.filename, "File Type": frontend_file_type}), 200
            elif frontend_file_type == ".obj":
                #Pass Frontend File Data and File Type to frontend_upload_file Function
                result = frontend_upload_file(frontend_file, frontend_file_type)
                is_file_uploaded = True
                ...
                print("File Uploaded Successfully")
                print(frontend_file.filename)
                return jsonify({"Message": ".obj File Data Received Successfully", "File Name": frontend_file.filename, "File Type": frontend_file_type, "Uploaded Data": result}), 200 #DEBUG FOR .csv FILES!!!!!
        except Exception as e:
            print(f"Error: {str(e)}")
            return jsonify({"Error": str(e)}), 500
    elif request_type == 'graph-data': #Column Data Selection Update
        ...
        #Loop Through Filtered Data To Determine Data Types
        for Values in json_filtered_data1: #JSON Filtered Data 1 Loop
            global value_type1 #Declare Variable to Store Column Data Type
            if isinstance(Values, (int, float)):
                value_type1 = "Number" #Set Type Equal to JSON Compatible Message
        ...
        for Values in json_filtered_data2: #JSON Filtered Data 2 Loop
            global value_type2 #Declare Variable to Store Column Data Type
            if isinstance(Values, (int, float)):
                value_type2 = "Number"
            elif isinstance(Values, (str)):
                value_type2 = "String"
        ...
        for Values in json_filtered_data3: #JSN Filtered Data 3 Loop
            global value_type3 #Declare Variable to Store Column Data Type
            if isinstance(Values, (int,float)):
                value_type3 = "Number"
            elif isinstance(Values, (str)):
                value_type3 = "Unknown"
        ...
        return jsonify({ #Send Column Data for .csv Files to Frontend
                ...
                "Data Type for Column 1": value_type1,
                "Data Type for Column 2": value_type2,
                "Data Type for Column 3": value_type3,
        }), 200
        ...
        print("Frontend Analyze Data Result: ", result)
        if result is None:
            ...
            return jsonify({"mean_result": mean_result }), 200
        else:
    ...
    elif request_type == 'display-data-post':
        #Parse JSON data from the request body
        data = request.get_json()
        ...
        result = frontend_display_data(column_name1, column_name2, display)

        if result is None:
            return jsonify({"Error": "Data Display Yielded no Results."})
        return jsonify({
...
def csv_listener_Get():
    ...
    elif request_type == 'get-data-indices': #Data Indices Selection
        try:
            print("Data Indices Sent.")
        except Exception as e:
            print(f"Error: {str(e)}")
        return jsonify(data_indices = list(data_indices)) #Send Data Indices to Local Server in JSON format
    #Display Data
...
def launch_backend_ui():
    ...
    def analyze_data():
        ...
        if column_name1 and column_name1 in uploaded_data.columns and test_name == "Mean": #Mean Calculation (FUNCTIONALITY REMOVED??? NEED UPDATED NUMPY FOR TENSORFLOW COMPATIBILITY)
            #Mean calculation attempt
            try:
                filtered_data1 = uploaded_data[column_name1].dropna() #Filter
                print("Data Analyzed")
                print("The mean of the selected data is: ", np.mean(filtered_data1)) #Numpy Mean Function (FUNCTIONALITY REMOVED??? NEED UPDATED NUMPY FOR TENSORFLOW COMPATIBILITY)
                messagebox.showinfo("Mean: ", np.mean(filtered_data1)) #(FUNCTIONALITY REMOVED??? NEED UPDATED NUMPY FOR TENSORFLOW COMPATIBILITY)
            except Exception as e:
                print(f"Error analyzing data: {e}")
